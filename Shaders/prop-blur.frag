...
float calc_blur_alpha(float pixel_angle, float angle, float blur_angle, float alpha_factor) {
	return clamp((1 - abs((pixel_angle - angle - blur_angle) / blur_angle)) * alpha_factor, 0.0, alpha_factor);
}

void main() {
	float rpm = clamp(rpm - blur_onset_rpm, 0.0, blur_full_rpm);
	float blur_angle = clamp(rpm / ((blur_full_rpm - blur_onset_rpm) / (360 / num_blades)), 0.0, 360.0) * sense;
	...
	if (d <= 0.25) {
		alpha_factor = mix(1.0, blade_width_0 * num_blades / (2 * 3.14159265359 * d * (rpm / 60) * 0.04), d * 4);
	} else {
		if (d <= 0.5) {
	...
	while (angle * sense < 540.0) {
		if ((angle * sense <= pixel_angle * sense && pixel_angle * sense <= (angle + blur_angle) * sense)) {
			base.a = alpha_factor;
		} else if (((angle + blur_angle) * sense <= pixel_angle * sense) && (pixel_angle * sense <= (angle + blur_angle + blur_angle) * sense)) {
			base.a = calc_blur_alpha(pixel_angle, angle, blur_angle, alpha_factor);
		} else if (angle * sense - (360.0 * sense / num_blades) + blur_angle * sense <= pixel_angle && angle * sense - min(blur_angle * sense, 10) * sense <= pixel_angle * sense && pixel_angle * sense <= angle * sense) {
			base.a = mix(alpha_factor, calc_blur_alpha(angle + (360.0 / num_blades - 10) * sense, angle, blur_angle, alpha_factor), abs((pixel_angle - angle) / min(blur_angle * sense, 10)));
		}
